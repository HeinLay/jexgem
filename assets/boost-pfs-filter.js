...
ProductGridItem.prototype.compileTemplate = function(data) {
  ...
  itemHtml = itemHtml.replace(/{{itemDate}}/g, data.published_at);
  itemHtml = itemHtml.replace(/{{itemHandle}}/g, data.handle);
  itemHtml = itemHtml.replace(/{{itemVendorLabel}}/g, data.vendor);
  const virtual_fitting = Utils.getProductMetafield(data, "my_fields", "virtual_fitting");
  if (virtual_fitting == 'true') {
    itemHtml = itemHtml.replace(/{{camera}}/g,'<div class="camera-img"></div>');
  }
  else {
    itemHtml = itemHtml.replace(/{{camera}}/g,"");
  }
  ...
  const published_date = new Date(published).getTime()/ 1000;
  const date_finished = published_date + 604800;
  if ( date_finished > date_now) {
    itemHtml = itemHtml.replace(/{{NewLabel}}/g,'<span class="badge-new">NEW</span>');
  }
  else {
    itemHtml = itemHtml.replace(/{{NewLabel}}/g,"");
  }
  return itemHtml;
...
function buildImages(data) {
  ...
  var dataSrcSet = '',
    dataWidths = '[180, 360, 540, 720, 900, 1080, 1296, 1512, 1728, 2048]',
    dataSizes = 'auto',
    imgAlt = data.title,
    flipImageSrcSet = '';
  ...
  for (var i = 0; i < rangeWidths.length; i++) {
    dataSrcSet += Utils.getFeaturedImage(images, rangeWidths[9] + 'x') + ' ' + rangeWidths[9] + 'w';
    dataWidths += rangeWidths[i];

    if (activeSwapImage) {
      flipImageSrcSet += Utils.optimizeImage(images[1]['src'], rangeWidths[9] + 'x') + ' ' + rangeWidths[9] + 'w';
    }
  ...
  html += '<img class="boost-pfs-filter-product-item-main-image lazyload Image--lazyLoad"' +
    ...
    'data-src="' + Utils.getFeaturedImage(images, rangeWidths[9] + 'x') + '" ' +
  ...
  if (activeSwapImage) {
    html += ' data-img-flip-src="' + Utils.optimizeImage(images[1]['src'], rangeWidths[9] + 'x') + '" ' +
...
function buildPrice(data) {
  ...
  if (boostPFSThemeConfig.custom.hasOwnProperty('show_price') &&
    ...
    if (onSale) {
      html += '<s>' + data.compare_at_price_min + '</s> ';
...
function buildLabels(data) {
  ...
  // Build Labels
  return soldOutLabel + saleLabel + salePercent;
}
